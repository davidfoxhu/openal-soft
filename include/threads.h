...
static __inline althrd_t althrd_current(void)
{
    return GetCurrentThreadId();
}

static __inline int althrd_equal(althrd_t thr0, althrd_t thr1)
{
    return thr0 == thr1;
}

static __inline void althrd_exit(int res)
{
    ExitThread(res);
}

static __inline void althrd_yield(void)
{
    SwitchToThread();
}


static __inline int almtx_lock(almtx_t *mtx)
{
...
static __inline int almtx_unlock(almtx_t *mtx)
{
    if(!mtx) return althrd_error;
...
static __inline int almtx_trylock(almtx_t *mtx)
{
    if(!mtx) return althrd_error;
...
static __inline void *altss_get(altss_t tss_id)
{
    return TlsGetValue(tss_id);
}

static __inline int altss_set(altss_t tss_id, void *val)
{
...
static __inline int althrd_sleep(const struct timespec *ts, struct timespec *rem)
{
...
static __inline void alcall_once(alonce_flag *once, void (*callback)(void))
{
